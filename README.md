...
- Список поддерживаемых опций:
  ...
  - `-c | --color-mode` - включает цветной режим вывода имен файлов.
  
  - `-C | --colorless-mode` - включает бесцветный режим вывода имен файлов.
  
  - `-s | --safe-mode` - включает безопасный режим вывода имен файлов.
    Подробнее: `jlsMakeStringSafe()`
  
  - `-S | --unsafe-mode` - включает небезопасный режим вывода имен файлов.
...
Для создания тестовой среды выполните:
...
- Выравнивание владельца и группы работает некорректно, если владелец или группа файла не имеет строкового вида. Обнаружено под `ls` версии `9.1` на Debian 12
